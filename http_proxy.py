...
def proxy(client_socket,client_IP):
                ...
                header = str(raw, 'utf-8')
                server_ip, server_port, hostname, connect = parse_server_info(header)
                data = modify_headers(header)

                host = bytes(hostname , 'utf-8')
                ...
                if connect:
                    ...
                    t = threading.Thread(target=tunnel, args=(client_socket,server_socket))
                    t.start()
                    s = threading.Thread(target=tunnel, args=(server_socket,client_socket))
                    s.start()
                    create_log2(hostname, raw, response)
                ...
                if not connect:
                    ...
                    create_log(hostname,raw,data,server_response)
                    client_socket.close()
                    server_socket.close()
